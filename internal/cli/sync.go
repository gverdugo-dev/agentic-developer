...
func (c syncCmd) Run(args []string) error {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(fs.Output(), "harness's own CLI; missing skills are copied from a discovered copy of")
		fmt.Fprintln(fs.Output(), "the same name (the cross-harness skill installer). Extra artifacts")
		fmt.Fprintln(fs.Output(), "(present but undeclared) are reported and never deleted.")
